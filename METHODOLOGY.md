...
**版本**: v1.2 — **RATIFIED**（PM 终审 2026-07-12）。修订走 PM ratify（§7）。
...
- **条件分布前置**：任何「情景 X 是否需要新规则」的问题，第一步 = 统计该情景在信号的**真实条件分布**中的出现次数；分布中不存在（n=0）的状态不建 production rule。判例：Execution Timing——「触发后 T+1 gap>1%」26 年零次，讨论就地终结（2026-07-30，外审 concur）。合理直觉先过存在性检验，再谈规则化。
...
| 摩擦按腿实测 | 交易成本按腿型实测半价差（深 ITM ~0.2% vs OTM ~1.0%），禁平摊常数；摩擦经笔数不对称直接进 head-to-head Δ（1.6% 平摊曾虚增挑战者 ~$56k 完全翻转 verdict） | Q089 |
| MDE 分级 null 语言 | 每个 null 结论必须附最小可检测效应量（80% 功效）；区分"有功效的参数性 null"与"功效不足无裁决"——后者不是 kill，带数据条件性重开 | Q090 外审 |
| 选择算术优先于叙事 | 菜单选择产生的"最强候选"先算 P(max\|t\|≥观测 \| 全局 null)（Q090：18 组合 ≈30%）；能被选择算术解释的不再造市场故事 | Q090 外审 |
...
- **字段单位约定验证先于写入**：向任何数字字段写入前先读该字段的消费方代码/测试夹具定单位（每股 vs 总美元 vs 百分比）——grandfather 补录 fill_debit 按每股填入总美元字段致结算 2× 虚增，交叉核对拦截（2026-07-11）
- **派生状态面尊重源字段语义**：消费 payload 时必须按语义字段过滤（is_active/settled/voided），"对象存在"≠"状态成立"——已结算历史仓被渲染成 HOLD 实例（2026-07-11）
- **探针先校准已知阳性**：任何扫描/查询类结论（grep、DOM 查询、日志检索）出"零命中/全空"前，先对一个已知存在的阳性验证探针本身——引号截断 grep、class-vs-id 选择器、异步渲染等待不足三例同课（2026-07-08~12）
...
- **States-vs-levels 范围纪律**：二元状态信号的 kill 不得外推为对"价位类主张"的裁决（组织轴不同）；kill 档案写明被杀的字面翻译边界（Q090：F2 状态 kill ≠"压力位不存在"）
- **新闻剔除 = outcome-conditioning，禁止**：用结果期归因筛样本即用答案改考卷；unscheduled 事件无事前清单。合法替身 = 中位数/截尾稳健统计 + 事前已知日历（FOMC/CPI/earnings）分层（Q090 方法学裁决）
...
- **负结果的正面价值**：「证明不用改」是合法且高价值的 verdict。效应量不支持时（判例：执行时点全部自由度 2.4% vs 表达选择 3.7-7.4×），正确输出 = **关闭声明 + 预注册重开条件**（防周期性重提），而非继续压榨；减少系统复杂度本身计为收益（2026-07-30，外审 concur）。
...
- **验收语义自洽扫描**：任何面向 PM 的输出（页面/推送/表格）验收时逐行检查视觉身份（图标/颜色/位置/分组）与文字主张一致、分子分母同池——"⛔ 配 不拦操作"与"50.28% 混池"双实例，PM 一晚连抓（2026-07-07）
- **多会话共享仓纪律**：并行会话期间 dev 交付一律 worktree 隔离 + 推分支不碰 main 不部署；集成者验收后合并、统一部署（部署顺带发布共享 main 上全部已验证提交是正常流转）；混提交事故（26 文件）为反例判例（2026-07-08）
- **测试密闭性**（SPEC-130 宪法化引用）：测试触碰的外部副作用通道（telegram/ledger/broker 写/缓存）必须走 conftest 隔离清单；传输层 deny-by-default 主机 guard；"测试污染生产资源"二次浮面后的结构化防线
...
*v1.2 2026-07-12 并入 Q089/Q090/工程周十课：按腿摩擦、MDE 分级 null、选择算术、states-vs-levels、outcome-conditioning 禁令、字段单位验证、派生面语义过滤、探针校准、语义自洽验收、多会话仓纪律 + 测试密闭性宪法化引用（PM ratify 2026-07-12）*
*v1.3 2026-07-30 并入 DD Overlay 表达线两课：条件分布前置（§1）、负结果的正面价值（§4）——PM 与外审三轮 review 共同结晶（PM ratify 2026-07-30 "上升成了一套比较成熟的研究方法"）*
