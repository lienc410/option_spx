...
2. `Q036` capital-allocation 主线推进
   - 扫描原文写法：`PM 选择 Option B ESCALATE`
...
所有四个 toggle 参数默认值必须保持 `disabled`：
