...
**Update 2026-05-15 (Q072 CLOSED — APPROVED + SPEC-103 APPROVED by PM)**: P4C.0–P4C.6 + closure complete. Key findings: (1) **Priority allocator ≡ FCFS in 19y simulation**——4 个非 static-cap allocator (main-first / sleeve-first / FCFS / priority) 给出完全一致的 total P&L $742k / max DD -$175k / Sharpe 3.36，因为 sleeve cadence 自然错开，无 BP 竞争实例。(2) **Static per-sleeve cap 反而减少 $102k P&L 且未改善 max DD**——destroys winners without protecting tail. (3) **R6 second-leg block 是真正高价值规则**——19y 仅 fire 26 天，但 2022 stress B_tight 比 default 减少 $11.6k 组合损失。(4) Walk-forward Spearman 0.704 (borderline overfit) — moot 因为 priority allocator 无价值。**Final verdict**: Augmented Default Cap（R1-R4 默认 + R5 stress episode SPX cap reduce to 60% + R6 second-leg short-vol absolute block）。**Do not implement priority allocator. Do not implement static per-sleeve caps.** **SPEC-103 APPROVED by PM 2026-05-15** — awaiting Developer implementation (F1-F6: portfolio state tracker / state flag computation / entry gate / logging / dashboard / override CLI; AC1-AC9; `task/SPEC-103.md`). Deferred validation (不阻塞 SPEC-103 实施): P4B /ES pool rerun + P4C.7 full synthetic stress test (2008/2022 inject sleeve pack)，待 Q071 HV Ladder final config lock 后做。Q072 final memo: `research/q072/q072_final_memo_2026-05-15.md`. 2 rounds 2nd Quant review both PASS w/ revisions, final verdict APPROVE.

**Update 2026-05-15 (SPEC-103 DONE + DEPLOYED old Air)**: Developer 完成实施并部署到 old Air，commit 1c690cc。实现内容：新增 `strategy/sleeve_governance.py` + `scripts/sleeve_governance_daemon.py` + `scripts/manage_governance.py` + `/api/sleeve-governance/state` 端点 + Portfolio Command Center "Sleeve Stress Governance" panel。`tests/test_spec_103.py` 8/8 PASS（本地 + old Air），含 **AC8 backtest replay smoke test 数值 100% 匹配 Q072 P4C.4 default cap**（n_entered=872 / total_pnl=$742,193 / max_dd=-$174,959）—— 这是 SPEC-103 设计中 Quant joint validation gate，实施已通过。`/api/sleeve-governance/state` / `/api/portfolio/summary` / `/api/recommendation` 在 old Air 都返回 200。com.spxstrat.web 与 com.spxstrat.bot 已重启。state tracker 已运行一次写入 `data/sleeve_governance_state.jsonl`。Handoff: `task/SPEC-103_handoff.md`. Q072 / SPEC-103 议题正式从 Quant lane 完全交付——下一 Quant 介入节点：Q071 HV Ladder final config lock 后做 deferred validation (P4B /ES rerun + P4C.7 full synthetic 2008/2022 stress inject)。已知非本次问题：相邻回归 tests.test_spec_089 仍有 2 个既有 E-Trade 测试失败（与 SPEC-103 无关）。

**Update 2026-05-16 (口径修正 + Cap recalibration — 70% 维持但理由换)**: PM 观察到 governance panel SPX PM BP 13.4% 与 Portfolio Snapshot total_real_margin 20.1% 不一致——发现 SPEC-103 R1 实施使用 `spx_live_bp_pct`（SPX spread max-loss only）而非 account-level maintenance margin（与 §R1 设计意图 "SPX 账户内所有持仓总和" 不符）。**(1) 口径 fix (commit 0fae2e4)**：`strategy/sleeve_governance.py` 改读 `account_breakdown.schwab_maintenance_margin` / `etrade_maintenance_margin`，新增 `pools_by_view = {all, schwab, etrade}` 与 Portfolio Snapshot toggle 联动；前端 `_renderGovernance` 按 `_accountView` 切换；tests 9/9 PASS。**(2) Cap recalibration**：跑 baseline-sensitivity（research/q072/q072_p4_cap_recalibration.py），结论：原 Q072 brief 说 "70% cap based on backtest peak 67.9%" 是 sleeve-only 口径，但 production daemon 用 account-level maintenance。在当前 PM 持仓 baseline (Schwab equity 14.4% NLV) 下，sleeve peak + baseline = 82.3%（超 Schwab PM call 80% 触发线），70% cap 在 19y 中会 bind 5 天——**cap 真的会工作，不是过松**。**Verdict**: 维持 70% cap 但物理依据从 "backtest peak + 2pp safety" 改为 "Schwab PM call 80% - 10pp safety margin"。SPEC-103.md 加 cap caveat 段落，明确两种口径不可直接比较，baseline 是 dynamic 测量；若 PM 未来增持股票 baseline > 25% 需重审。R3 Combined cap 60% 也存在类似 binding 加重风险，标 future-watch 但不在本次改。Output: `research/q072/q072_p4_cap_recalibration_2026-05-16.md` + `q072_p4_cap_recalibration.csv`. No service restart needed (docs only).
...
### R-20260712-02 — Q097 P3: 确认期 K 扫描 + 斜通道冒烟（PM 边界追问双测，CLOSED 2026-07-12）

- **触发**: SPEC-142 事件流展品暴露两个现象——2/8 确认后数日即破箱（PM 问 15TD 可否缩短）、5/7 破箱为平移再锚定（PM 问斜通道可否避免）
- **K 扫描裁定**: **维持 15**。速死率对 K∈{8..20} 恒定 30-35%（箱体确认后中位寿命 ~10TD 与 K 无关，缩短不治速死只加推送）；K<12 杀 K3 崩盘证书（3/4）；K=12 合法但负期望（+4 推送/年、2 触发分型漂移、速死率不降）
- **斜通道裁定**: **不立项**。现象诊断成立（历史 130 次再锚定、82% 上移），但 4/4 崩盘触发日前夜均存在干净斜通道（残差带 2.7-4.8%）→ 通道检测器 K3 0-1/4 级死亡，比 D2(2/4) 更惨；且吸收测试显示其同时吞掉 2026-03-13 真裂口推送。Q097 理论点第二实例: **不适应性=崩盘识别能力**（带宽自适应死于迁就动荡，斜率自适应死于迁就漂移）
- **零风险替代**: SPEC-142 文案附破箱方向（向上/向下，当日可知，F2 合规）——82% 上移再锚定归 benign 类，降噪价值零几何变更拿到大半 → 并入 SPEC-142 草案
- **文件**: q097_p3_ksweep_channel.py + 两 CSV + findings §6

### R-20260712-03 — Q100 P1: DD Overlay 双 sleeve 独立重验（PM 指令"不默认旧研究可信"，CLOSED 2026-07-12）

- **触发**: PM 直接指令——独立重验当前双 sleeve 配置（A: dd-4/T+1/ATM+2.5%/D30/12.5%→17.5%；B: dd-15+MA10/ATM+5%/D90/0%+paper10%）是否设计空间最优
- **独立性**: 状态机自实现（vs 生产 walk-forward A 38/38 B 5/5 全一致）；旧 CSV 逐笔复现中位差 2.3% 无系统漂移；定价三路 INC/CALIB/STRESS，结算 model-free 内在价值；预注册 R1-R4 判定规则先于跑数
- **核心发现（定价仲裁）**: 真实链锚点（2026-05-04 SPX.parquet）判 **CALIB 全点位胜 INC**（|err| ≤4.8% vs ≤11.8%），且 INC 误差 width-directional——低估 2.5% 腿 debit 5.7%、高估 5% 腿 11.8% → **Q062 选 2.5% 宽度的"AnnROE 持平"是相对偏 ~17pp 的尺子量出来的**；F4 tie-out 全部 5 行为旧结构（D84-88），**当前生产结构 D30/2.5% 从未做过真实链对账**
- **裁定**: ①触发 -4%、T+1 立即入场、DTE30 **独立确认现任**（挑战者 bootstrap P 全 <0.90；dip alpha 集中前 4 周，$/cash-kday D30 21-27 vs D60/90 7-11）；②**宽度 2.5%→5% 通过全部预注册 R1**（INC/CALIB/STRESS 三路 Δ>0：+54k/+292k/+599k；boot P=0.965；post-2020 同向；$/cash-day +29%；风险轴同尺下尾部完全相同、2.5% 仅剩 WR+5pp 装饰性优势；STRESS 档另暴露窄腿对定价误差 10× 脆弱）；③B facts-only（n=5）：100% WR 是 settle 运气（±3 周 jitter 掉 60-80%）、immediate 变体不劣且更稳、**re-arm -2% 使 B 每轮周期单发**（2008-10/2009-03/2011/2022-10 深渊全部错过）——5%/90 维持，re-arm 设计问题登记不立项；④**17.5% staging 目标算术不可执行**（+1 live BCD = 98% 池、+2 = 123% 抽穿），挂起绑定 Q092 (a)/(b)
- **路由**: 宽度改 5% 待 PM ratify（094.1 镜像小 spec，AC 必须含新结构真实链 tie-out ≥5 日——顺带补对账空白）；D30/2.5% tie-out 无论如何该补；B/12.5% 维持
- **文件**: research/q100/ q100_p1_replay.py + q100_p1b_jitter_h2h.py + findings + 7 CSV

### R-20260712-04 — Q101 E1: Aftermath 结构独立复审（PM 指令"不默认旧研究可信"，外审 CONFIRMED 2026-07-12，待 PM ratify）

- **触发**: PM 直接指令——独立复审生产 aftermath 通道的 V3-A broken-wing 是否最合适；旧研究（Q064 P3）三缺口：n=15、候选集=2、σ=VIX 平坦零 skew（早于 CALIB 宪法）
...
### R-20260730-05 — Q100 P3：−4% 阈值细网格 sweep（PM 挑战"硬线还是大致范围"，CLOSED 2026-07-30）

- **挑战成立**: −4% 系 1pp 网格区域代表，从未测 −3.85/−3.9；−3.86 vs −4.00 之别在阈值分辨率下
- **Sweep（0.05pp，预注册禁移线）**: [−3.5,−4.5] 全域高原（Σ $1,103-1,531k，效率 24-32），相邻格点差 = 1-2 笔事件噪音；−3.90 局部峰为 2014 路径拟合物，不可碰
- **Near-miss 队列**: 26 年 4+1 次（~6.5 年/次），21TD 结果 3 涨 1 跌（2015-12 那次随后真触发即满损）——非系统性错过
- **裁定**: −4% 身份改写为「高原上的治理常数」（与 094.8 15% 同句式）；不开枪的辩护 = 线的固定性非数字最优性；事后移线 = 裁量漂移开始。代码注释/findings/log 三处落档

### R-20260730-06 — 高原稳健性检验：PM 二次挑战成立，一处修正 + Group C 首例登记（CLOSED 2026-07-30）

- **挑战**: 关闭结论部分依赖"昨天没触发"的约定判定（循环）→ 三表重跑四个可容许约定（−3.85~−4.00，数据同步至 07-30）
- **站住**: recovery rule 全约定 +EV（+$352-492k, WR 67-75%——约定无关）；gap 分层单调性保持；不移线
- **修正**: 执行时点杠杆 2.4%→**2.4-9.1%**（原引最有利约定值；结论不变且强化：延迟在部分约定下更贵）；packet 关闭声明定量句已改
- **新事实**: −3.85 约定下 **2026-07-29 = 26 年首个 Group C 事件**（影子触发 gap +1.02%）——"场景不存在"修正为"历史不存在，首例刚发生、结果待定"；30d 结果 9/1 入档（DEFERRED #25）；影子触发规则候选 n≥8 重看
- **裁量边界**: 无信号日裁量入场不因高原视角正当化（rule-shopping 禁令）；生产纪律层不受高原影响——它守的是固定性不是知识

**R-20260730-06 补验（PM 三问："带内收盘+大gap 只此一次吗"）**: 无条件扫描（26y 全部 dd∈[−4.5,−3.5] 收盘日）找到 6 例"带内+次日 gap>1%"；逐一状态验证——4 例已在仓内（早数日真触发已抓住该 dip，gap 为持仓顺风：2020-11-04/2024-05-02/2024-07-30/2025-01-14）、1 例假高开次日照常触发（2025-11-19→11-20）、**07-29 为 26y 唯一"armed 空仓 + near-miss + 大 gap + 此后未触发"完整组合**。结构性事实：深跌区暴力高开多发生在持仓期（trigger+re-arm 节奏使然）——07-29 缝隙需三条件同时成立（底部停在 ~10 点窗口 + 无前仓 + 次日反转），26 年一遇，不值得规则填补。"首个 Group C"表述经此验证成立。

### R-20260805-01 — Q106：两个月市场-策略契合度独立复盘（CLOSED 2026-08-05）

- **市场**: 46 TD 磨盘（+1.6%，区间 6.5%，RV 15.3% vs VIX 中位 17 → VRP 薄 +1.7vp；两次急跌 V 收复），期末 **08-04 收盘 7736.52 新 ATH 强上破**（+1.66% 超箱顶，非弱探型）
- **三层判定**: 策略层全对（near-miss 按设计、新 ATH 正确升级）；执行层利用率≈0（06-05 后 44 TD 零新开仓，premium 引擎主战场整季未开火，窗口已实现仅 +$1.8k）；基础设施层 8 周 5 类事件——①ATH 腐蚀[已修] ②gate degraded×2 ③**E-Trade liquid 静默塌陷×3（B1 家族第 5 例，喂 094.8 现金分支）** ④NLV fetch $0（08-05）⑤**结构轴 7 天宽限尾致 142 首次真实破箱通知未发**（显示轴误继承崩盘分类器语义，RANGE→RANGE 换箱可永久吞通知）
