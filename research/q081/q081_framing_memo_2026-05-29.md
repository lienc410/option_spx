...
### **P3** — QQQ / SGOV hurdle benchmark（同 trade window）

**问题**：BCD 同期若改投 QQQ（或现金留 SGOV），实际回报多少？

**计算**：
- 对每笔 BCD 的 entry → exit 区间，计算 QQQ 同期 return
- 对每笔 BCD 同期 hold days，计算 SGOV yield（约 4.8% / 365 × days）
- 报 hurdle 分布的 p05 + 均值
- BCD net = BCD cash-ROE - QQQ hurdle（per trade）

**输出**：`research/q081/q081_p3_hurdle_benchmark.{py,csv,md}`

**Decision logic**:
- BCD p05 cash-ROE > QQQ p05 hurdle by **at least** ?bp（待 G-review 1 设定）→ BCD 跑赢
- BCD p05 cash-ROE ≤ QQQ p05 hurdle → BCD 没跑赢，倾向 conclusion 2（gate）
