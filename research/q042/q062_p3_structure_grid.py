"""Q062 — P3: Structure comparison, per-sleeve independent.

Sleeve A: dd4 (n=25/26 from q042_backtest_trades.csv)
Sleeve B: dd15 + MA10 reclaim (n=5)

Fixed DTE = 90 (P1/P2 winner for both sleeves).
Compare four structures:
  A: ATM/+5% call spread (current SPEC-094)
  B: Long ATM call only
  C: Far-OTM call spread: ATM×1.07 / ATM×1.15 (lottery ticket)
  D: ATM/+10% call spread (wide spread)

No-overlap rule: max 1 active position per sleeve.

Output: research/q042/q062_p3_structure_grid.csv
"""
...
OUT_CSV = Path(__file__).resolve().parent / "q062_p3_structure_grid.csv"
...
STRUCTURES = [
    {"label": "A_spread_atm_5pct", "name": "ATM/+5% spread (SPEC-094)", "kind": "spread", "long_mult": 1.00, "short_mult": 1.05},
    {"label": "B_long_atm_call", "name": "Long ATM call only", "kind": "long_call", "long_mult": 1.00, "short_mult": None},
    {"label": "C_far_otm_spread", "name": "Far-OTM spread +7%/+15%", "kind": "spread", "long_mult": 1.07, "short_mult": 1.15},
    {"label": "D_spread_atm_10pct", "name": "ATM/+10% spread (wide)", "kind": "spread", "long_mult": 1.00, "short_mult": 1.10},
]
...
def price_structure(S: float, vix: float, dte: int, kind: str, long_mult: float, short_mult: float | None) -> tuple[float, float]:
    """Returns (debit_per_share, bp_at_risk).
    For spreads: bp_at_risk = debit.
    For long call: bp_at_risk = debit.
    """
    T = dte / 365.0
    K_long = S * long_mult
...
def compute_payoff(S_T: float, S: float, kind: str, long_mult: float, short_mult: float | None) -> float:
    K_long = S * long_mult
    if kind == "long_call":
        return max(0.0, S_T - K_long)
    K_short = S * short_mult
    long_pay = max(0.0, S_T - K_long)
    short_pay = max(0.0, S_T - K_short)
    return long_pay - short_pay
...
def compute_metrics(
    ...
    kind = structure["kind"]
    long_mult = structure["long_mult"]
    short_mult = structure["short_mult"]
    ...
    for sd in filtered:
        ...
        debit, bp_at_risk = price_structure(S, vix_val, dte, kind, long_mult, short_mult)
        ...
        if status == "OPEN":
            ...
            payoff = compute_payoff(latest_S, S, kind, long_mult, short_mult)
        else:
            payoff = compute_payoff(S_T, S, kind, long_mult, short_mult)
            if kind == "spread" and short_mult is not None:
                short_strike_hit = (S_T >= S * short_mult)

        pnl = payoff - debit
        pnl_pct = pnl / bp_at_risk * 100  # % of BP
    ...
    # max_loss_per_bp_pct = 100% of debit for all debit structures (long call and spreads)
    max_loss_per_bp_pct = 100.0
    ...
    return {
        ...
        "max_loss_per_bp_pct": max_loss_per_bp_pct,
    }
...
def main() -> None:
    ...
    for sleeve, dates in signal_dates.items():
        for structure in STRUCTURES:
            metrics = compute_metrics(dates, spx_df, vix_s, DTE, structure, sleeve)
            rows.append(metrics)
            print(f"Sleeve {sleeve} {structure['label']}: "
                  f"n={metrics['n_filtered']}, "
    ...
    for sleeve in ["A", "B"]:
        ...
        print(sub[[
            "structure_name", "n_filtered", "win_rate", "median_pnl_pct",
