...
def run_phase2_hvlad(
    ...
    # SPEC-121 (PM 2026-07-05): canonical 10x everywhere — the ungated
    # comparison column (vix_min_entry<=0) changes vs its 15x history
    # (17 stop exits appear); PM accepted the rewrite. The gated ladder is
    # bit-identical under 10x (147 trades, 0 stops, verified).
...
def run_phase2_v2f(
    ...
    # SPEC-121 (PM 2026-07-05): canonical stop 10x applies EVERYWHERE, incl.
    # this unfiltered baseline — PM knowingly accepted the baseline history
    # rewrite (under 10x it is not bit-identical to the old 15x: 5 -> 29 stop
    # exits, bootstrap sig_rate 0.85 -> ~0; see test_spec_095 for the pin).
    result = _run_phase2_v2f_on_frame(
        sim,
        full_spx,
        mode=mode,
