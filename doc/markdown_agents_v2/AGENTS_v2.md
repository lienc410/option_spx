...
- **Planner（ChatGPT 或其他低成本模型）**：负责维护项目状态、整理研究结论、生成候选任务与优先级；不做最终策略设计
- **你（Codex）**：只执行，不设计
...
- **路径 C（研究/规划）**：Claude 输出研究结论 → Planner/PM 整理优先级与项目状态 → PM 决定是否进入 Spec
...
说明：
- 路径 C 不直接触发生产代码修改
- 只有当 PM 将任务写入 `task/SPEC-{id}.md` 且设为 `Status: APPROVED` 后，你才开始实施
...
规则：
- 你（Codex）默认不修改 `RESEARCH_LOG.md`
- 若 PM 明确要求，且任务已写入 APPROVED Spec，你可按 Spec 更新 `PROJECT_STATUS.md`
- 未进入 APPROVED Spec 的研究结论，不由你自行落地为代码
