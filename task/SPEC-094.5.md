...
| AC# | 描述 | 结果 |
|---|---|---|
| AC-94.5-1 | `compute_sizing(500k, 7400, 25, "A")` → strikes (7400, 7770)，DTE 30 语义不变 | ✅ test_spec_094_5 |
| AC-94.5-2 | Sleeve B 零 diff：`compute_sizing(..., "B")` strikes/DTE/debit 与改动前逐位一致 | ✅ 冻结值 pin ×2 |
| AC-94.5-3 | 再生 CSV：A 行 short_strike 全部 = round5(S×1.05)；**信号流对齐**：A 行 signal_date 集合 == live 状态机 walk-forward（38 事件，feedback_signal_translation_alignment_ac） | ✅（引擎统一后逐位相等；顺带修出引擎漏 2 笔到期日再触发） |
| AC-94.5-4 | 再生 CSV 金行复核：2007-02-27 行 (KL 1400, KS 1470, exit_pnl = 内在−debit 手算一致)；B 行与再生前逐位一致 | ✅ |
| AC-94.5-5 | F4 补账：old Air 最近 5 交易日 D≈30 两宽度对账行落盘；PASS = 5 日中位 \|delta_pct\| < 15%（沿 F4 门槛）；record 预注册区间命中与否 | ❌ **FAIL 15.69%**（差 0.69pp）：+5% 行 delta −6.5~−17.0（model 贵于 broker，保守向——sizing 少开 ~14% 合约，无危害向风险）；预注册区间 [−15,−5] 命中 40%；+2.5% 参照行 +0.5~+5.6 全 PASS。诊断 = INC 粗 skew 对宽腿的规律性 rich 偏差（Q100 §2 同向，且 VIX 15 低位是其最不利 regime）。**后续路由：INC→CALIB 估价迁移立 spec 候选（PM 排期）**；期间生产照跑（错误方向保守） |
| AC-94.5-6 | 回归：094.2(22)+094.3(13)+094.4(13)+142(14)+094.5(4)+094.6(4) 全绿；`/api/q042/state` params 块 otm_pct 显示 5.0 | ✅ 70 passed；old Air 端点实测 otm_pct 5.0 / otm_mult 1.05 |

**部署验证（2026-07-12）**：old Air pull + web 重启；引擎再生 43 行（A 37+1 OPEN / B 5）；executor 生产日志 dry-run：`ath=7609.78 ddath=-0.45%`、gate main_bp=15.8% 真值、零误触发。首次实弹 = 下一次 ddATH ≤ −4%（以 +5% 结构开仓）。
...
---
Status: DEPLOYED 2026-07-12 (old Air verified; AC-94.5-5 FAIL 如实记录 → INC→CALIB 迁移候选)
