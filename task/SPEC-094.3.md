...
- **T+2**（entry_target_date 后 2 个交易日仍 null）→ FYI 提醒（gateway，dedupe `q042_fill_reminder_{trade_id}`）："Q042 [sleeve] 触发单未确认成交——已执行请回填 fill_debit；未执行将于 T+5 自动释放槽位"。
- **T+5** 仍 null → **释放槽位**：清 `state[sleeve].active_position_id`/`active_position_expiry` + 在该账本记录上打 `phantom: true` 标记（不删记录，保审计链）+ ACTION 告警："幽灵仓位已释放，sleeve 恢复可触发；若实际已成交请立即补录并手工恢复 state"。
