...
_UP_BREAK_LINE = ("历史 63% 破箱为上破，多数 ~3 周内再锚定新箱；"
                  "已知 4 次崩盘触发均发生在弱上破后 5–10TD 内"
                  "（上破后 20TD 出现崩盘触发的历史条件概率 3.3%）")
_DOWN_BREAK_LINE = ("历史 37% 破箱为下破；20TD 尾部风险重于上破"
                    "（dd≤−7% 占 14% vs 7%）——本类含 2026-03-13 型真裂口")
_RANGE_AMMO_PREFIX = "历史 89% 的 dip 触发源于此状态 → 弹药检查："
