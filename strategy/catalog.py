...
STRATEGIES_BY_KEY: dict[str, StrategyDescriptor] = {
    ...
    "bull_call_diagonal": StrategyDescriptor(
        ...
        detail_roll_text="Close the entire position when the short leg reaches 21 DTE. Take profit at +60% of debit (params.profit_target); stop at -50% of debit. If the short leg's remaining value decays to ≤15% of the credit you received, buy it back (or roll to a new short) immediately — at that point the leftover premium no longer pays for the gamma risk of keeping it. The bot pushes a CLOSE/ROLL alert when this triggers.",
        max_risk_text="Net debit paid",
        target_return_text="15–20% of debit paid; close entire position at 60% profit",
        roll_rule_text="Close entire position at 21 DTE of short leg; stop out at 50% loss of debit; buy back the short leg once its value decays to ≤15% of credit received",
        short_gamma=False,
        short_vega=False,
        delta_sign="bull",
    ),
    ...
    "bull_put_spread": StrategyDescriptor(
        ...
        detail_roll_text="30 DTE 入场，短腿走到 21 DTE 时无条件平仓——约 13-14 个日历日（9 个交易日）。此窗口短于 profit-target 的 10 日最短持有门槛，因此 60% 止盈在本策略上不激活（Q107 P1 实证：26 年 0 次触发；测过放宽反而更差）。仍然生效：2× credit 止损、roll-up 管理。",
        max_risk_text="Spread width − net credit (defined, capped downside)",
        target_return_text="持有至短腿 21 DTE 平仓收取时间价值（非止盈退出）",
        roll_rule_text="Close at 21 DTE（时间退出，非止盈）；stop at 2× credit",
        short_gamma=True,
        short_vega=True,
        delta_sign="bull",
    ),
