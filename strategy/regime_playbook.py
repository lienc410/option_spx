        ...
        },
        {
            ...
            "lines": [
                {"line": "第一个动作是清点抄底资金，不是加对冲——抄底触发线（距历史高点 "
                         ...
                         "开盘执行",
                 "ref": "Q093 / SPEC-094.7"},
                {"line": "执行信号 = 收盘后 ~16:15 的开火告警（ACTION，带 strikes/张数/"
                         "预估成本）；盘中推送只是态势感知，不是发令枪——触发判定只看收盘"
                         "（盘中穿线收盘收回 = 无信号；日内触发方案测过：提前率 0%）。"
                         "次日开盘照进，跳空方向不改变执行（历史 gap 中位 +0.03%；"
                         "等回调 = 系统性更差）",
                 "ref": "SPEC-030 / Q089 E2"},
